...
from business.agents.information_gathering_agent.prompt_templates.v2.information_gathering_agent import (
    INFORMATION_GATHERING_AGENT_SYSTEM_PROMPT_V2
)
from business.agents.information_gathering_agent.tools import store_summary, store_home_issue
...
class InformationGatheringAgent:
    ...
    def get_checkpointer(self):
        ...
        with MongoDBSaver.from_conn_string(self.settings.MONGODB_URI) as checkpointer:
            yield checkpointer

    def process_text_response(self, message: str, thread_id: str) -> str:
        """
        ...
        """
        logger.info(f"Processing text response for thread_id: {thread_id}")
        ...
        try:
            with self.get_checkpointer() as checkpointer:
                ...
                agent = create_agent(
                    ...
                    system_prompt=INFORMATION_GATHERING_AGENT_SYSTEM_PROMPT_V2,
                    checkpointer=checkpointer,
                )

                config: RunnableConfig = {
                    "configurable": {"thread_id": thread_id, "recursion_limit": 20}
                }
    ...
    def process_image_response(self, text: Optional[str], image_base64: str, mime_type: str, thread_id: str) -> str:
        """
        ...
        Returns:
            Agent's response text
        """
        logger.info(f"Processing image response for thread_id: {thread_id}")
